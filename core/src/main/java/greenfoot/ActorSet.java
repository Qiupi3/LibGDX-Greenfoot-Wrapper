...
public class ActorSet extends AbstractSet<Actor> {
    ...
    public ActorSet() {
        // Constructor - ListNode constructor creates the head/tail node
    }
    ...
    public boolean add(Actor actor) {
        ...
        if (hashMap.length == 0 || numActors >= 2 * hashMap.length) {
            // grow the hashmap
            resizeHashmap();
        }
        else {
    ...
    private void resizeHashmap() {
        int newSize = Math.max(1, numActors);
        hashMap = new ListNode[newSize];
    ...
    private void remove(ListNode actorNode) {
        ...
        actorNode.remove();
        numActors--;
        if (numActors > 0 && numActors <= hashMap.length / 2) {
            // shrink the hashMap
            resizeHashmap();
        }
    }
